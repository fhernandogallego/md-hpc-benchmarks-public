...
RUN_ROOT="${1:?Usage: submit_gwo_improved_v2.sh RUN_ROOT}"

PROJECT="$HOME/md-hpc-benchmarks"

SBATCH="$PROJECT/slurm/gwo_improved_v2_array.sbatch"
...
cp \
    "$PROJECT/configs/gwo_search_space_v2.json" \
    "$RUN_ROOT/provenance/"
...
analysis=final_improved_gwo_expanded_space
...
(
    ...
    sha256sum \
        gwo_improved_v2_array.sbatch \
        gwo_search_space_v2.json \
