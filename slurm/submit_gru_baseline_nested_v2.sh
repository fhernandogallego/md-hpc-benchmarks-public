...
RUN_ID="atlas11_gru_baseline_nested_v2_2026-08-12"

OUTPUT_ROOT="$WORK/results/modeling/$RUN_ID"

TRAINER="$PROJECT/src/modeling/train_gru_baseline.py"
SBATCH_SOURCE="$PROJECT/slurm/gru_baseline_nested_v2_array.sbatch"
SUBMITTER_SOURCE="$PROJECT/slurm/submit_gru_baseline_nested_v2.sh"
...
cp \
    "$SBATCH_SOURCE" \
    "$PROVENANCE/gru_baseline_nested_v2_array.sbatch"

cp \
    "$SUBMITTER_SOURCE" \
    "$PROVENANCE/submit_gru_baseline_nested_v2.sh"
...
FROZEN_SBATCH="$PROVENANCE/gru_baseline_nested_v2_array.sbatch"
...
(
    ...
    sha256sum \
        ...
        gru_baseline_nested_v2_array.sbatch \
        submit_gru_baseline_nested_v2.sh \
        run_config.txt \
        fold_manifest.tsv \
