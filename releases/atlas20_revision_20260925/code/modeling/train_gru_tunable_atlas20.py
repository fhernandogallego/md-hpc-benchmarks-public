...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train deterministic leakage-free tunable GRU "
...
def main() -> None:
    ...
    # PyTorch GRU dropout is applied only between recurrent layers.
    # For a single-layer GRU the effective dropout is therefore
    # exactly zero, preserving baseline equivalence.
    effective_dropout = (
        args.dropout
    ...
    if (
        ...
        raise RuntimeError(
            "Unexpected ATLAS20 train/validation "
            "sample-count pairing."
        )

    if (
        y_train_np.shape[0]
        != X_train_np.shape[0]
        or y_valid_np.shape[0]
        != X_valid_np.shape[0]
    ):
        raise RuntimeError(
    ...
    metadata = {
        ...
        "data": {
            ...
            "n_valid": int(X_valid_np.shape[0]),
            "train_target_counts": (
                mask_train_np.sum(
                    axis=0
                ).astype(int).tolist()
